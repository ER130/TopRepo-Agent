...
**0. Locate (or clone) TopRepo.** Look for an existing clone at
`<project_root>/toprepo/` first. If it's not there, clone it:
```
git clone https://github.com/toppic-suite/toprepo <project_root>/toprepo
```
Everything below writes `<toprepo_dir>` for wherever that ends up --
resolve it once at the start of a run, don't assume the path.
...
`<toprepo_dir>/src/process/msalign/msalign_reader.py` subclasses
...
```
python3 <toprepo_dir>/src/process/mzml/extract_mzml_info.py <dataset_id> <input>.mzML <dataset_id>_<mzml_stem>_mzml_info.tsv
```
...
```
python3 <toprepo_dir>/src/process/msalign/extract_msalign_info.py <dataset_id> <input>_ms2.msalign <name>_msalign_info.tsv
```

**1.3 Extract feature info**
```
python3 <toprepo_dir>/src/process/feature/extract_feature_info.py <dataset_id> <input>_ms2.feature <name>_feature_info.tsv
```
...
```
python3 <toprepo_dir>/src/process/tsv/merge_mzml_msalign_toppic_info.py <name>_msalign_info.tsv <name>_feature_info.tsv <name>_toppic_info.tsv <name>_file_info.tsv <name>_combined_info.tsv
```

- The **4th argument is not a free choice** -- the README's own example
  points it at `<toprepo_dir>/resources/toprepo_file_info_v1.2.1.tsv`, but
...
```
python3 <toprepo_dir>/src/process/msalign_anno/msalign_preprocess.py <input>_ms2.msalign <dataset_id> <name>_ms2_preprocess.msalign
```

**2.2 Merge in the PrSM identification** -- needs `PYTHONPATH` set to
`<toprepo_dir>/src`, or it fails with `ModuleNotFoundError: No module named
'process'` (verified: this is true exactly as the README's own command is
written, regardless of your working directory):
```
PYTHONPATH=<toprepo_dir>/src python3 <toprepo_dir>/src/process/msalign_anno/merge_msalign_prsm.py \
...
```
PYTHONPATH=<toprepo_dir>/src python3 <toprepo_dir>/src/process/msalign_anno/msalign_anno_based_frequency.py \
    --msalign <name>_ms2_prsm.msalign --table <toprepo_dir>/resources/toprepo_ion_freq_v1.2.1.tsv --out <name>_anno_ms2.msalign
```
...
```
python3 <toprepo_dir>/src/process/mzml/convert_mzml_to_mgf.py <input>.mzML <name>_ms2.mgf
python3 <toprepo_dir>/src/process/mgf/mgf_add_dataset_id.py <name>_ms2.mgf <dataset_id> <name>_dataset_id_ms2.mgf
python3 <toprepo_dir>/src/process/mgf/mgf_anno_file.py \
    --theo_file <toprepo_dir>/resources/theo_patt.txt \
...
`<toprepo_dir>/script/*.sh` has folder-level wrappers (`extract_folder_*.sh`,
...
| Copying TopRepo's scripts into this repo instead of running them from the clone | Duplicates code this project doesn't own/maintain and will drift from upstream | Resolve `<toprepo_dir>` (prerequisite 0) and always invoke scripts from there in place |
...
**Upstream (not vendored here -- see Prerequisites)**:
- `https://github.com/toppic-suite/toprepo` -- clone and run in place
- `<toprepo_dir>/README.md` -- the original 3-phase walkthrough this skill
  is built on top of (with the two step-1.5 gaps and the import/dependency
  issues above filled in)
- `<toprepo_dir>/resources/toprepo_ion_freq_v1.2.1.tsv` -- required `--table`
  for step 2.3
- `<toprepo_dir>/resources/theo_patt.txt` -- required `--theo_file` for
  step 3.3
